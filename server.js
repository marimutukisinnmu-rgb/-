...
function countTerritory(id) {
  let count = 0;
  for (const owner of territory) if (owner === id) count++;
  return count;
}
...
function summary() {
  return [...players.values()].sort((a, b) => a.id - b.id).map(p => ({
    ...
    alive: p.alive, territory: countTerritory(p.id), weapon: p.weapon, weaponCooldown: 0
  }));
}

function projectileSummary() {
  return [...projectiles.values()].map(p => ({
    id: p.id, owner: p.owner, type: p.type, x: p.x, y: p.y,
    angle: p.angle, radius: p.radius, emoji: p.emoji
  }));
}

function stateMessage() {
  return JSON.stringify({
    type: 'state', grid: { w: GRID_W, h: GRID_H }, territory: Array.from(territory),
    players: summary(), projectiles: projectileSummary(), started: gameStarted, winner
  });
}

function broadcast(payload) {
  const message = typeof payload === 'string' ? payload : JSON.stringify(payload);
  for (const player of players.values()) if (player.ws.readyState === 1) player.ws.send(message);
}
...
function resetTerritory() {
  ...
  for (const player of players.values()) {
    ...
    player.homeX = Math.floor(x); player.homeY = Math.floor(y);
    player.angle = 0; player.weapon = 'potato';
    paintHome(player);
  }
}
...
function startNewRound() {
  ...
  winner = null; gameStarted = players.size >= 2;
  resetTerritory(); broadcastState();
}

function eliminateIfEmpty() {
  for (const player of players.values()) if (player.alive && countTerritory(player.id) === 0) player.alive = false;
  const alive = alivePlayers();
  if (gameStarted && alive.length === 1) {
    winner = alive[0].id; gameStarted = false; broadcastState();
...
function rainfall() {
  for (const player of players.values()) {
    for (let i = 0; i < territory.length; i++) if (territory[i] === player.id) territory[i] = -1;
    territory[idx(player.homeX, player.homeY)] = player.id;
    player.x = player.homeX + 0.5; player.y = player.homeY + 0.5; player.angle = 0; player.alive = true;
  }
  broadcast({ type: 'rain' }); broadcastState();
}

function blackButton(sourcePlayer) {
  const centerX = sourcePlayer.x, centerY = sourcePlayer.y;
  for (const player of players.values()) {
    if (!player.alive) continue;
    player.x = wrap(centerX + Math.floor(Math.random() * 21) - 10, GRID_W);
    player.y = centerY; player.angle = 0;
  }
  broadcast({ type: 'blackButton', x: centerX, y: centerY }); broadcastState();
}
...
function applyWeaponArea(ownerId, x, y, radius) {
  paintCircle(x, y, radius, ownerId); eliminateIfEmpty();
}

function mysteryWeapon(source) {
  const roll = Math.floor(Math.random() * 4);
  if (roll === 0) {
    for (const player of players.values()) if (player.alive && player.id !== source.id) applyWeaponArea(source.id, player.x, player.y, 6);
    broadcast({ type: 'weaponEvent', kind: 'mystery', text: '☢️ 謎のボタン：敵の周囲を一気に塗った！' });
  } else if (roll === 1) {
    rainfall(); broadcast({ type: 'weaponEvent', kind: 'mystery', text: '☢️ 謎のボタン：なぜか雨が降った！' });
  } else if (roll === 2) {
    blackButton(source); broadcast({ type: 'weaponEvent', kind: 'mystery', text: '☢️ 謎のボタン：全員整列！' });
  } else {
    for (const player of players.values()) player.angle = 0;
    broadcast({ type: 'weaponEvent', kind: 'mystery', text: '☢️ 謎のボタン：全員右向き！' }); broadcastState();
  }
}
...
function fireWeapon(player) {
  ...
  if (!weapon) return;
  // クールダウンなし。呼ばれるたび即発射。
  if (weapon.mystery) { mysteryWeapon(player); return; }
  for (let i = 0; i < weapon.count; i++) {
    const spread = weapon.count > 1 ? (i - (weapon.count - 1) / 2) * weapon.spread : 0;
    const angle = player.angle * Math.PI / 180 + spread;
    const id = crypto.randomBytes(8).toString('hex');
    projectiles.set(id, {
      id, owner: player.id, type: player.weapon, x: player.x, y: player.y,
      angle, speed: weapon.speed, radius: weapon.radius,
      homing: !!weapon.homing, explode: !!weapon.explode,
      emoji: player.weapon === 'potato' ? '🥔' : player.weapon === 'chicken' ? '🐔' :
        player.weapon === 'chair' ? '🪑' : player.weapon === 'sock' ? '🧦' :
...
function updateProjectiles(dt) {
  for (const [id, projectile] of projectiles) {
    ...
    if (projectile.radius > 0) paintCircle(projectile.x, projectile.y, projectile.radius, projectile.owner);
    projectileHitsPlayer(projectile);
    if (projectileReachedScreenEdge(projectile)) projectiles.delete(id);
  }
}

function createPlayer(ws) {
  if (players.size >= MAX_PLAYERS) {
    ws.send(JSON.stringify({ type: 'error', message: '満員です（最大10人）' })); ws.close(); return null;
  }
  const id = assignPlayerId(); if (id === null) return null;
  const [x, y] = spawnPositions[id];
  const token = crypto.randomBytes(16).toString('hex');
  const player = {
    token, id, color: COLORS[id], ws, x: x + 0.5, y: y + 0.5, angle: 0,
    alive: true, homeX: Math.floor(x), homeY: Math.floor(y), weapon: 'potato', lastTurnAt: 0
  };
  players.set(token, player); paintHome(player); if (players.size >= 2) gameStarted = true;
  ws.send(JSON.stringify({ type: 'welcome', token, player: { id: player.id, color: player.color, x: player.x, y: player.y, angle: player.angle, weapon: player.weapon } }));
  broadcastState(); return player;
}
...
const server = http.createServer((req, res) => {
  ...
  if (!fullPath.startsWith(root + path.sep)) { res.writeHead(403); res.end('Forbidden'); return; }
...
wss.on('connection', (ws) => {
  const player = createPlayer(ws); if (!player) return;
  ws.on('message', (raw) => {
    let data; try { data = JSON.parse(raw.toString()); } catch { return; }
    if (!player.alive) return;
    if (data.action === 'turn') {
      const now = Date.now(); if (now - player.lastTurnAt < TURN_INTERVAL_MS) return;
      const direction = Number(data.direction); if (direction !== -1 && direction !== 1) return;
      player.angle = (player.angle + TURN_ANGLE * direction + 360) % 360; player.lastTurnAt = now;
    } else if (data.action === 'rain') rainfall();
    else if (data.action === 'blackButton') blackButton(player);
    else if (data.action === 'weapon') { const weapon = String(data.weapon); if (WEAPONS[weapon]) player.weapon = weapon; }
    else if (data.action === 'fire') fireWeapon(player);
  });
  ws.on('close', () => {
    players.delete(player.token);
    for (const [id, projectile] of projectiles) if (projectile.owner === player.id) projectiles.delete(id);
...
setInterval(() => {
  const now = Date.now(); const dt = Math.min((now - lastTick) / 1000, 0.1); lastTick = now;
  movePlayers(dt); updateProjectiles(dt); eliminateIfEmpty(); if (gameStarted) broadcastState();
}, 1000 / TICK_RATE);
