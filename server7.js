...
// server5.js is itself a wrapper around server4.js. Inject patches into the
// actual server source immediately before server5 compiles it.
const marker = "const loaded = new Module(target, module);\nloaded.filename = target;";
if (!source.includes(marker)) throw new Error('server5 wrapper marker not found');
...
for (const [from, to] of aptxPatches) {
  if (!source.includes(from)) throw new Error('APTX patch point not found: ' + from.slice(0, 90));
  source = source.replace(from, to);
}
`;

source = source.replace(marker, patchCode + '\n' + marker);
