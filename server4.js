...
function resetRound() {
  ...
  for (const p of players.values()) {
    ...
    p.x = x;
    p.y = y;
    p.homeX = Math.floor(x);
    p.homeY = Math.floor(y);
    p.angle = 0;
    p.weapon = 'potato';
    p.nextMysteryAt = 0;
    paintHome(p);
...
function adminRandomSpecial() {
  ...
  if (source) blackButton(source);
    ...
    for (const p of players.values()) p.angle = 0;
...
function createHuman(ws) {
  ...
  ws.send(JSON.stringify({
    type: 'welcome',
    token,
    player: { id: p.id, color: p.color, x: p.x, y: p.y, angle: p.angle, weapon: p.weapon, cpu: false }
  }));
  broadcastState();
  return p;
}
...
function createCPU() {
  ...
  const p = {
    id,
    token: `cpu-${crypto.randomBytes(10).toString('hex')}`,
    cpu: true,
    ws: null,
    color: colorFor(id),
    x,
    ...
    homeX: Math.floor(x),
    homeY: Math.floor(y),
    angle: Math.floor(Math.random() * 24) * 15,
    alive: true,
    weapon: WEAPON_KEYS[Math.floor(Math.random() * (WEAPON_KEYS.length - 1))],
    nextMysteryAt: 0,
    lastTurnAt: 0
  };
  players.set(id, p);
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/admin' && req.method === 'GET') {
    if (!isLoopback(req)) return sendJson(res, 403, { ok: false, message: 'admin is localhost only' });
    const file = path.join(root, 'static', 'admin.html');
    return fs.readFile(file, (err, data) => {
      if (err) { res.writeHead(500); return res.end('Admin page unavailable'); }
  ...
  if (url.pathname === '/admin/login' && req.method === 'POST') {
    ...
    return sendJson(res, 200, { ok: true }, {
      'Set-Cookie': `adminSession=${encodeURIComponent(sid)}; HttpOnly; SameSite=Strict; Path=/`
    });
  }
  ...
  if (url.pathname === '/admin/action' && req.method === 'POST') {
    ...
    if (action === 'rain') {
      rainfall();
      return sendJson(res, 200, { ok: true, action: 'rain' });
    }
    if (action === 'blackButton') {
      const source = alivePlayers()[0];
      if (source) blackButton(source);
      else {
        for (const p of players.values()) p.angle = 0;
      ...
      return sendJson(res, 200, { ok: true, action: 'blackButton' });
    }
    if (action === 'random') {
      return sendJson(res, 200, { ok: true, action: adminRandomSpecial() });
    }
    return sendJson(res, 400, { ok: false, message: 'unknown admin action' });
  }
  ...
  if (url.pathname === '/api/cpu' && (req.method === 'POST' || req.method === 'DELETE')) {
    ...
    return sendJson(res, 200, {
      ...
      cpus: [...players.values()].filter(p => p.cpu).length,
      maxPlayers: MAX_PLAYERS
    });
  }
  ...
  if (url.pathname === '/api/status' && req.method === 'GET') {
    ...
    return sendJson(res, 200, {
      ok: true,
      players: players.size,
      humans: [...players.values()].filter(p => !p.cpu).length,
      cpus: [...players.values()].filter(p => p.cpu).length,
      maxPlayers: MAX_PLAYERS
    });
  }
...
wss.on('connection', ws => {
  ...
  if (!p) {
    ws.send(JSON.stringify({ type: 'error', message: `満員です（最大${MAX_PLAYERS}人）` }));
    ws.close();
    return;
  }
